...
- Shipped `src/adapters/eleventy.js` (`./eleventy` export) — `elementSSR(opts)` returns an Eleventy `addTransform` function. It's a **non-arrow** function on purpose (reads `this.page.outputPath`) and gates on `(this.page?.outputPath || "").endsWith(".html")` so it only transforms HTML output (feeds/JSON/`permalink:false` pass through); on `.html` it calls the shared `renderToString`. Shape-4 framing, shape-2 mechanics — verified against 11ty 3.x docs. `test/eleventy.test.js` (4): DSD render, non-HTML + `false`-outputPath passthrough, lazy resolve loads only what's present, `serializeState` script. **Suite green at 66.**
- Added `examples/eleventy/` — a **content-driven** Eleventy 3.x site (ESM config): Markdown content (`src/index.md`) through a Nunjucks layout (`src/_includes/base.njk`), reusing the Astro `x-counter`/`x-greeting` components + element-library's `./catalog`. SSR-only (mirrors `examples/node`) to isolate the build-time adapter. Resolution = `[catalog, ./_catalog.js]` where `_catalog.js` is generated by `element-js-ssr-renderer catalog` (Eleventy is Node, no `import.meta.glob`), wired to `prebuild`/`predev`. **Verified** `npm install && npm run build`: `_site/index.html` has 8 DSD templates, the `<x-greeting>` authored in the **Markdown content** pre-rendered (Eleventy's default `html: true`), attribute-seeded `Apples: 3`, `el-button` from the catalog, and the nested button inside `el-notification`. Static top-level import order sufficed (no rollup-hoist gotcha, unlike Nitro).
- Docs: new `docs/frameworks/eleventy.md` (incl. transform-order + minifier caveat, and client-hydration pointing at the Node guide); flipped the support-matrix row to ✅ shape 4; sidebar entry; `examples/README.md` + root README updated; README "Framework integrations" link repointed to `/frameworks/`. Docs build green. Per the reframe, documented honestly as a _second_ shape-4 example justified on the **authoring-model** axis (content-SSG vs Vite's static-HTML app).
...
- Converted **all four** examples' SSR resolution to the wrapper-free, library-ships-its-own-catalog pattern: deleted the eager `import Button/Notification` + `{ "el-button": Button, … }` blocks and replaced them with `import catalog from "@webtides/element-library/catalog"` → `resolve: [catalog, <own components>]`. Astro/SvelteKit/Vite use the static `import` (Vite preserves order); Nuxt uses a dynamic `import()` inside the Nitro plugin (eval-order), keeping its generated local catalog. The plain-Vite example (T-015.8) was included — it had the same eager block. Bumped each example's `@webtides/element-library` to `^0.2.0`.
- **Verified end-to-end** — installed 0.2.0, built, and SSR-served each example: Astro 9 / SvelteKit 9 / Nuxt 8 (curled) and Vite 9 (built `dist/index.html`) DSD templates, with `el-button` (from the library catalog) and `x-counter` (local) both rendering. The Nuxt/Nitro pass confirms rollup traces the catalog's package-internal `() => import("./src/components/…")` specifiers.
