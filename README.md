...
`renderToStringAsync` directly — buffering the page's chunks and transforming the whole document once,
on the final chunk:
...
`elementSSR` takes the same sources as everywhere else (see
[Loading & resolving components](#loading--resolving-components)). Author components as plain HTML in
your `+page.svelte`/`+layout.svelte`, keep document-global styles in `app.html` (so the renderer can
adopt them into shadow roots), and load each component's `define` on the client (e.g. from a layout's
`onMount`).

A complete, runnable version lives in [`examples/sveltekit/`](./examples/sveltekit) — an
`@sveltejs/adapter-node` app composing element-library components (eager registry) with its own
