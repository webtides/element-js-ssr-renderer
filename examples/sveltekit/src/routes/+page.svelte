<svelte:head>
  <title>element-js SSR renderer — SvelteKit example</title>
</svelte:head>

<h1>element-js SSR renderer — SvelteKit example</h1>
<p>
  Every custom element below is authored as plain HTML in this
  <code>+page.svelte</code>. The <code>handle</code> hook (from the
  <code>./sveltekit</code> adapter) pre-renders each one on the server — shadow
  components as Declarative Shadow DOM, light-DOM components in place — then they
...
<p>
  ...
  <code>&lt;template shadowrootmode="open"&gt;</code> for the shadow ones. Then
  with JS on, the counter's buttons work: that's hydration, not a re-render.
</p>
...
<section>
  ...
  <!-- Light DOM. -->
  <x-greeting name="SvelteKit"></x-greeting>
</section>
